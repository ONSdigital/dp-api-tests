...
func TestGetVersion_ReturnsSingleVersion(t *testing.T) {
	...
	Convey("Create a dataset", t, func() {
		...
		Convey("Get a list of all editions of a dataset", func() {
			...
			Convey("Get an edition of a dataset", func() {
				...
				datasetAPI.GET("/datasets/{id}/editions/{edition}", datasetID, edition).WithHeader("internal-token", "FD0108EA-825D-411C-9B1D-41EF7727F465").WithBytes([]byte(validPOSTCreateInstanceJSON)).
					Expect().Status(http.StatusOK)

				Convey("Get a list of all versions from an edition of a dataset", func() {

					datasetAPI.GET("/datasets/{id}/editions/{edition}/versions", datasetID, edition).WithHeader("internal-token", "FD0108EA-825D-411C-9B1D-41EF7727F465").WithBytes([]byte(validPOSTCreateInstanceJSON)).
						Expect().Status(http.StatusOK)
